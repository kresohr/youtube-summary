...
app.listen(PORT, () => {
  ...
  console.log(`[${new Date().toISOString()}] Running initial video fetch on startup...`);
  ...
  cron.schedule('0 5 * * *', () => {
    console.log(`[${new Date().toISOString()}] Cron triggered: daily video fetch`);
    fetchAndSummarizeVideos().catch((error) => {
      console.error('Cron fetch error:', error);
    });
  });

  console.log(`[${new Date().toISOString()}] Cron job scheduled: daily at 05:00`);
});
